...
| A18 | Constant-product `x*y=k` pricing with a fee taken from the input | CONFIRMED | Public mathematics, implemented exactly. Any *specific* protocol's router or encoding remains UNKNOWN |
| A19 | Gas cost of a two-swap arbitrage transaction (250k units) and per-chain fee levels | UNCERTAIN | Assumed and marked `confirmed=False` in code. Read-only measurement can promote these |
| A20 | Inclusion probability 0.60 and revert probability 0.25 | UNCERTAIN | Assumed, pessimistically. The sweep shows the result degrades gracefully in both, so precision here is not critical |
| A21 | Competitors close ~35% of any pool divergence per block | **UNCERTAIN — and decisive** | Assumed. At 0.50 the strategy places no trades at all. This single unmeasured number decides whether DEX/DEX exists. See DEX_RUN_REPORT.md §5 |
| A22 | Pool divergence distribution (median 0.31%, p90 0.77%) | UNCERTAIN | Generated by a chosen process, not observed. It sets the entire opportunity frequency |
| A23 | An atomic round trip that reverts leaves the position unchanged | CONFIRMED | Follows from transaction atomicity, and is what `exposure_delta` relies on. Only valid while both swaps genuinely share one transaction |
